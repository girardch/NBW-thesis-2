...
# (Assuming your glm_model_5 and glm_data have been created as in your GLM code)
# Compute predicted values for each observation (on the response scale)
glm_data[, pred_edge := predict(glm_model_5, type = "response")]

# Define custom colors for dyadic age and dyadic sex
custom_colors_age <- c(
...
### Boxplot for dyadic_age with manual significance annotations
# GLM coefficients indicate:
#   Adult_Calf vs Adult_Adult: p = 0.80612 ("ns")
#   Adult_Juvenile vs Adult_Adult: p = 3.43e-06 ("***")
#   Calf_Juvenile vs Adult_Adult: p = 0.02684 ("*")
#   Juvenile_Juvenile vs Adult_Adult: p = 0.00887 ("**")
p1_box <- ggplot(glm_data, aes(x = dyadic_age, y = pred_edge, fill = dyadic_age)) +
  ...
  geom_signif(
    comparisons = list(
      c("Adult_Adult", "Adult_Calf"),
      c("Adult_Adult", "Adult_Juvenile"),
      c("Adult_Adult", "Calf_Juvenile"),
      c("Adult_Adult", "Juvenile_Juvenile")
    ),
    annotations = c("ns", "***", "*", "**"),
    y_position = c(0.9, 0.95, 1.0, 1.05)
  )

### Boxplot for dyadic_sex with manual significance annotations
# GLM coefficients indicate (with reference "FemaleJ_FemaleJ"):
#   FemaleJ_Male vs FemaleJ_FemaleJ: p = 3.28e-07 ("***")
#   Male_Male vs FemaleJ_FemaleJ: p = 0.01977 ("*")
p2_box <- ggplot(glm_data, aes(x = dyadic_sex, y = pred_edge, fill = dyadic_sex)) +
  geom_boxplot(outlier.shape = NA, alpha = 0.7) +
  ...
  geom_signif(
    comparisons = list(
      c("FemaleJ_FemaleJ", "FemaleJ_Male"),
      c("FemaleJ_FemaleJ", "Male_Male")
    ),
    annotations = c("***", "*"),
    y_position = c(0.9, 0.95)
  )
...
# Save the dyadic_age boxplot
ggsave(filename = "C:/Users/balae/OneDrive/Desktop/Claire NBW Thesis/p1_box.png", plot = p1_box, width = 8, height = 6, dpi = 800)

# Save the dyadic_sex boxplot
ggsave(filename = "C:/Users/balae/OneDrive/Desktop/Claire NBW Thesis/p2_box.png", plot = p2_box, width = 8, height = 6, dpi = 800)
...
# Plot for dyadic_age with adjusted x-axis labels
...
# Plot for dyadic_sex with adjusted x-axis labels
...
# If sj_glm_1 is a single ggplot object, modify its layers.
if (!is.list(sj_glm_1)) {
  # Loop over all layers and if a layer has a linetype, force it to "solid"
  for (i in seq_along(sj_glm_1$layers)) {
    if (!is.null(sj_glm_1$layers[[i]]$aes_params$linetype)) {
      sj_glm_1$layers[[i]]$aes_params$linetype <- "solid"
    }
  }
  ...
  # If sj_glm_1 is a list of plots, apply the modifications to each plot in the list.
  sj_glm_1 <- lapply(sj_glm_1, function(p) {
    for (i in seq_along(p$layers)) {
      if (!is.null(p$layers[[i]]$aes_params$linetype)) {
        p$layers[[i]]$aes_params$linetype <- "solid"
      }
    }
    p + labs(x = NULL, y = "HWI (Strength)") +
      theme(
        plot.title = element_blank(),
        panel.grid = element_blank()
      ) +
  ...
  invisible(lapply(sj_glm_1, print))
}
...
visualization
```{r}
# Assuming glm_data_filtered is already defined and filtered

# Create a new grouping variable ("Group")
# For Adult, append the Sex; for non-adults, just use Age_Class
glm_data_filtered[, Group := ifelse(Age_Class == "Adult",
                                    paste0("Adult ", Sex),
                                    as.character(Age_Class))]

# Set the order of levels (modify if needed)
glm_data_filtered[, Group := factor(Group, levels = c("Adult FemaleJ", "Adult Male", "Calf", "Juvenile"))]

# Plot using the new grouping variable (single panel, no facet)
p <- ggplot(glm_data_filtered, aes(x = Group, y = Degree, fill = Group)) +
  geom_boxplot(alpha = 0.7, outlier.shape = NA) +  # Exclude outliers
  scale_fill_manual(
    values = c("Adult FemaleJ" = "lightblue", 
               "Adult Male" = "pink", 
               "Calf" = "purple", 
               "Juvenile" = "darkblue")
  ) +
...
ggsave(filename = "C:/Users/balae/OneDrive/Desktop/Claire NBW Thesis/p.png", 
       plot = p, 
       width = 8, 
       height = 6, 
       dpi = 800)
...
# If the result is a list, apply modifications to each plot
if (is.list(sj_glm_3_nd)) {
  sj_glm_3_nd <- lapply(sj_glm_3_nd, function(p) {
    p + labs(x = NULL) + theme(plot.title = element_blank())
  })
}
...
```
want to determine the associates for each calf
